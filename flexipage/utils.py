...
# Only check for forms when the Django application is first started and this file is run
forms_dict = {}
try:
...
def get_flexi_forms(template_path):
    ...
    matched_forms = [x for x in forms_dict if x in template_forms]
    ...
    for form in matched_forms:
        ...
        unbound_forms[form_name] = forms_dict[form]
    return unbound_forms
    
def get_flexi_tags(template_name):
    """
    Returns a list of django template variable strings from the template `template_name`
    that start with FLEXI_VARIABLE_PREFIX.
    """
